...
class RegressionMetrics(Metrics):
    ...
    def log_nse(self, epsilon=0.0) -> float:
        return log_nse(true=self.true, predicted=self.predicted, epsilon=epsilon, preprocess=False)

    def log_prob(self) -> float:
        return log_prob(true=self.true, predicted=self.predicted, preprocess=False)
    ...
    def std_ratio(self, **kwargs) -> float:
        return std_ratio(true=self.true, predicted=self.predicted, preprocess=False, **kwargs)
...
def r2(true, predicted, preprocess: bool = True,
       **process_kws) -> float:
    """
    ...
    >>> r2(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['r2'], **process_kws)
...
def nse(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    ...
    >>> nse(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nse'], **process_kws)

    _nse = 1 - sum((predicted - true) ** 2) / sum((true - np.mean(true)) ** 2)
    return float(_nse)


def nse_alpha(true, predicted, preprocess: bool = True,
              **process_kws) -> float:
    """
    ...
    >>> nse_alpha(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nse_alpha'], **process_kws)
    return float(np.std(predicted) / np.std(true))


def nse_beta(true, predicted, preprocess: bool = True,
             **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nse_beta'], **process_kws)
    return float((np.mean(predicted) - np.mean(true)) / np.std(true))


def nse_mod(true, predicted, preprocess: bool = True, j=1,
            **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nse_mod'], **process_kws)
...
def nse_rel(true, predicted,
    ...
    >>> nse_rel(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nse_rel'], **process_kws)
...
def nse_bound(true, predicted, preprocess: bool = True,
              **process_kws) -> float:
    """
    ...
    >>> nse_bound(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nse_bound'], **process_kws)
    nse_ = nse(true, predicted, preprocess=False)
...
def r2_score(true, predicted, preprocess: bool = True, weights=None,
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['r2_score'], **process_kws)
    if len(predicted) < 2:
...
def adjusted_r2(true, predicted, preprocess: bool = True,
                **process_kws) -> float:
    """Adjusted R squared.
    ...
    >>> adjusted_r2(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['adjusted_r2'])
    k = 1
    n = len(predicted)
    adj_r = 1 - ((1 - r2(true, predicted, preprocess=False)) * (n - 1)) / (n - k - 1)
    return float(adj_r)
...
def kge(true,
    ...
    >>> kge(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['kge'], **process_kws)
...
def kge_bound(true, predicted, preprocess: bool = True,
              **process_kws) -> float:
    """
    ...
    >>> kge_bound(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['kge_bound'], **process_kws)
    kge_ = kge(true, predicted, return_all=True, preprocess=False)[0, :]
...
def kge_mod(true, predicted, preprocess: bool = True, return_all=False,
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['kge_mod'], **process_kws)
...
def kge_np(
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['kge_np'], **process_kws)
    # # self-made formula
    cc = spearmann_corr(true, predicted, preprocess=False)
...
def spearmann_corr(true, predicted, preprocess: bool = True,
                   **process_kws) -> float:
    ...
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['spearmann_corr'], **process_kws)
...
def log_nse(true, predicted, preprocess: bool = True, epsilon=0.0,
            **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['log_nse'], **process_kws)
...
def corr_coeff(true, predicted, preprocess: bool = True,
               **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['corr_coeff'], **process_kws)
    correlation_coefficient = np.corrcoef(true, predicted)[0, 1]
    return float(correlation_coefficient)


def rmse(true, predicted, preprocess: bool = True, weights=None,
         **process_kws) -> float:
    ...
    """
    if preprocess:
        true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rmse'], **process_kws)
    return sqrt(np.average((true - predicted) ** 2, axis=0, weights=weights))


def rmsle(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    ...
    >>> rmsle(t, p)
     ...
     """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rmsle'], **process_kws)
    return float(np.sqrt(np.mean(np.power(log1p(predicted) - log1p(true), 2))))


def mape(true, predicted, preprocess: bool = True,
         **process_kws) -> float:
    ...
    >>> mape(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mape'], **process_kws)
    return float(np.mean(np.abs((true - predicted) / true)) * 100)


def nrmse(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    ...
    >>> nrmse(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nrmse'], **process_kws)
    return float(rmse(true, predicted, preprocess=False) / (np.max(true) - np.min(true)))


def pbias(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    """
    Percent Bias.
    ...
    >>> pbias(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['pbias'], **process_kws)
    return float(100.0 * sum(predicted - true) / sum(true))


def bias(true, predicted, preprocess: bool = True,
         **process_kws) -> float:
    """
    ...
    >>> bias(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['bias'], **process_kws)
    bias_ = np.nansum(true - predicted) / len(true)
    return float(bias_)


def mae(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    """ Mean Absolute Error.
    ...
    >>> mae(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mae'], **process_kws)
    if true is None:
        true = true
...
def abs_pbias(true, predicted, preprocess: bool = True,
              **process_kws) -> float:
    ...
    >>> abs_pbias(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['abs_pbias'], **process_kws)
    _apb = 100.0 * sum(abs(predicted - true)) / sum(true)
    return float(_apb)


def gmae(true, predicted, preprocess: bool = True,
         **process_kws) -> float:
    ...
    >>> gmae(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['gmae'], **process_kws)
    error = true - predicted
    return _geometric_mean(np.abs(error))


def inrse(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['inrse'], **process_kws)
    error = true - predicted
    return float(np.sqrt(np.sum(np.square(error)) / np.sum(np.square(true - np.mean(true)))))


def irmse(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    ...
    >>> irmse(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['irmse'], **process_kws)
    ...
    return float(rmse(true, predicted, preprocess=False) / obs_grad_std)


def mase(true, predicted, preprocess: bool = True, seasonality: int = 1, **process_kws):
    """
    ...
    >>> mase(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mase'], **process_kws)

    return mae(true, predicted, False) / mae(true[seasonality:], _naive_prognose(true, seasonality), preprocess=False)


def mare(true, predicted, preprocess: bool = True, **process_kws) -> float:
    ...
    >>> mare(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mare'], **process_kws)
    error = true - predicted
    return float(np.mean(np.abs(error / true)))


def msle(true, predicted, preprocess=True, weights=None, **process_kws) -> float:
    """
    ...
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['msle'], **process_kws)
...
def covariance(true, predicted, preprocess: bool = True,
               **process_kws) -> float:
    """
    Covariance
        .. math::
    ...
    >>> covariance(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['covariance'], **process_kws)
...
def brier_score(true, predicted, preprocess: bool = True,
                **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['brier_score'], **process_kws)
...
def bic(true, predicted, preprocess: bool = True, p=1,
        **process_kws) -> float:
    """
    ...
    >>> bic(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['bic'], **process_kws)
    assert p >= 0

    n = len(true)
    return float(n * np.log(sse(true, predicted, preprocess=False) / n) + p * np.log(n))


def sse(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    """
    ...
    >>> sse(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['sse'], **process_kws)
    squared_errors = (true - predicted) ** 2
    return float(np.sum(squared_errors))


def amemiya_pred_criterion(true, predicted, preprocess: bool = True,
                           **process_kws) -> float:
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['amemiya_pred_criterion'],
                                       **process_kws)
    k = 1
    n = len(predicted)
    return float(((n + k) / (n - k)) * (1 / n) * sse(true, predicted, preprocess=False))


def amemiya_adj_r2(true, predicted, preprocess: bool = True,
                   **process_kws) -> float:
    ...
    """
    if preprocess:
        true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['amemiya_adj_r2'], **process_kws)
    k = 1
    n = len(predicted)
    adj_r = 1 - ((1 - r2(true, predicted, preprocess=False)) * (n + k)) / (n - k - 1)
    return float(adj_r)


def aitchison(true, predicted, preprocess: bool = True, center='mean',
              **process_kws) -> float:
    ...
    >>> aitchison(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['aitchison'], **process_kws)
    lx = log_t(true)
    ly = log_p(predicted)
...
def acc(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    ...
    >>> acc(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['acc'], **process_kws)
...
def agreement_index(true, predicted, preprocess: bool = True,
                    **process_kws) -> float:
    """
    ...
    >>> agreement_index(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['agreement_index'], **process_kws)
...
def aic(true, predicted, preprocess: bool = True, p=1,
        **process_kws) -> float:
    """
    ...
    >>> aic(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['aic'], **process_kws)
    assert p > 0
...
def cronbach_alpha(true, predicted, preprocess: bool = True,
                   **process_kws) -> float:
    """
    ...
    >>> cronbach_alpha(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['cronbach_alpha'], **process_kws)
...
def centered_rms_dev(true, predicted, preprocess: bool = True,
                     **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['centered_rms_dev'], **process_kws)
    # Calculate means
...
def cosine_similarity(true, predicted, preprocess: bool = True,
                      **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['cosine_similarity'], **process_kws)
    return float(np.dot(true.reshape(-1, ),
                        predicted.reshape(-1, )) /
                 (np.linalg.norm(true) * np.linalg.norm(predicted)))


def decomposed_mse(true, predicted, preprocess: bool = True,
                   **process_kws) -> float:
    """
    ...
    >>> decomposed_mse(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['decomposed_mse'], **process_kws)
    e_std = np.std(true)
    s_std = np.std(predicted)

    bias_squared = bias(true, predicted, preprocess=False) ** 2
    sdsd = (e_std - s_std) ** 2
    lcs = 2 * e_std * s_std * (1 - corr_coeff(true, predicted, preprocess=False))
...
def euclid_distance(true, predicted, preprocess: bool = True,
                    **process_kws) -> float:
    """Euclidian distance
    ...
    >>> euclid_distance(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['euclid_distance'], **process_kws)
    return float(np.linalg.norm(true - predicted))


def exp_var_score(true, predicted, preprocess: bool = True, weights=None,
                  **process_kws) -> Union[float, None]:
    """
    ...
    >>> exp_var_score(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['exp_var_score'], **process_kws)
...
def expanded_uncertainty(true, predicted, preprocess: bool = True, cov_fact=1.96,
                         **process_kws) -> float:
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['expanded_uncertainty'], **process_kws)
    error = true - predicted
    sd = np.std(error)
    return float(cov_fact * np.sqrt(sd ** 2 + rmse(true, predicted, preprocess=False) ** 2))


def fdc_fhv(true, predicted, preprocess: bool = True, h: float = 0.02,
            **process_kws) -> float:
    """
    ...
    >>> fdc_fhv(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['fdc_fhv'], **process_kws)
...
def fdc_flv(true, predicted, preprocess: bool = True, low_flow: float = 0.3,
            **process_kws) -> float:
    """
    ...
    >>> fdc_flv(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['fdc_flv'], **process_kws)
...
def gmean_diff(true, predicted, preprocess: bool = True,
               **process_kws) -> float:
    ...
    >>> gmean_diff(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['gmean_diff'], **process_kws)
    return float(np.exp(gmean(log1p(predicted)) - gmean(log1p(true))))


def gmrae(true, predicted, preprocess: bool = True, benchmark: np.ndarray = None,
          **process_kws) -> float:
    ...
    >>> gmrae(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['gmrae'], **process_kws)
...
def calculate_hydro_metrics(true, predicted, preprocess: bool = True,
                            **process_kws) -> dict:
    """
    ...
    metrics = {}
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['JS'], **process_kws)
...
def JS(true, predicted, preprocess: bool = True,
       **process_kws) -> float:
...
def kendaull_tau(true, predicted, preprocess: bool = True, return_p=False,
                 **process_kws) -> Union[float, tuple]:
    ...
    >>> kendaull_tau(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['kendaull_tau'], **process_kws)
...
def kgeprime_c2m(true, predicted, preprocess: bool = True,
                 **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['kgeprime_c2m'], **process_kws)
    kgeprime_ = kge_mod(true, predicted, return_all=True, preprocess=False)[0, :]
...
def kgenp_bound(true, predicted, preprocess: bool = True,
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['kgenp_bound'], **process_kws)
    kgenp_ = kge_np(return_all=True, true=true, predicted=predicted, preprocess=False)[0, :]
    kgenp_c2m_ = kgenp_ / (2 - kgenp_)

    return float(kgenp_c2m_)


def kl_sym(true, predicted, preprocess: bool = True,
           **process_kws) -> Union[float, None]:
    ...
    >>> kl_sym(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['kl_sym'], **process_kws)
...
def lm_index(true, predicted, preprocess: bool = True, obs_bar_p=None,
             **process_kws) -> float:
    ...
    >>> lm_index(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['lm_index'], **process_kws)
...
def maape(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    """
    ...
    >>> maape(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['maape'], **process_kws)
    return float(np.mean(np.arctan(np.abs((true - predicted) / (true + EPS)))))


def mbe(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    ...
    >>> mbe(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mbe'], **process_kws)
    error = true - predicted
...
def mbrae(true, predicted, preprocess: bool = True, benchmark: np.ndarray = None,
          **process_kws) -> float:
    ...
    >>> mbrae(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mbrae'], **process_kws)
    return float(np.mean(_bounded_relative_error(true, predicted, benchmark=benchmark)))


def mapd(true, predicted, preprocess: bool = True,
         **process_kws) -> float:
    ...
    >>> mapd(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mapd'], **process_kws)
...
def max_error(true, predicted, preprocess: bool = True,
              **process_kws) -> float:
    """
    ...
    >>> max_error(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['max_error'], **process_kws)
    return float(np.max(_ae(true, predicted)))


def mb_r(true, predicted, preprocess: bool = True,
         **process_kws) -> float:
    ...
    >>> mb_r(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mb_r'], **process_kws)
    # Calculate metric
    n = predicted.size
    tot = 0.0
    for i in range(n):
...
def mda(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    ...
    >>> mda(t, p)
     """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mda'], **process_kws)
    dict_acc = np.sign(true[1:] - true[:-1]) == np.sign(predicted[1:] - predicted[:-1])
    return float(np.mean(dict_acc))


def mde(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    """Median Error
    Parameters
    ----------
    ...
    >>> mde(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mde'], **process_kws)
...
def mdape(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    """
    ...
    >>> mdape(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mdape'], **process_kws)
    return float(np.median(np.abs(_percentage_error(true, predicted))) * 100)


def mdrae(true, predicted, preprocess: bool = True, benchmark: np.ndarray = None,
          **process_kws) -> float:
    ...
    >>> mdrae(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mean_bias_error'], **process_kws)
...
def me(true, predicted, preprocess: bool = True,
    ...
    >>> me(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['me'], **process_kws)
    error = true - predicted
    return float(np.mean(error))


def mean_bias_error(true, predicted, preprocess: bool = True,
                    **process_kws) -> float:
    """
    Mean Bias Error
...
def mean_var(true, predicted, preprocess: bool = True,
             **process_kws) -> float:
    """Mean variance
    Parameters
    ----------
    ...
    >>> mean_var(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mean_var'], **process_kws)
    return float(np.var(log1p(true) - log1p(predicted)))


def mean_poisson_deviance(true, predicted, preprocess: bool = True, weights=None,
                          **process_kws) -> float:
    """
...
def mean_gamma_deviance(true, predicted, preprocess: bool = True, weights=None,
                        **process_kws) -> float:
    """
    mean gamma deviance
    Parameters
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mean_gamma_deviance'], **process_kws)
    return _mean_tweedie_deviance(true, predicted, weights=weights, power=2)


def median_abs_error(true, predicted, preprocess: bool = True,
                     **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['median_abs_error'], **process_kws)
    return float(np.median(np.abs(predicted - true), axis=0))


def med_seq_error(true, predicted, preprocess: bool = True,
                  **process_kws) -> float:
    ...
    """
    if preprocess:
        true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['med_seq_error'], **process_kws)
    return float(np.median((predicted - true) ** 2))


def mle(true, predicted, preprocess=True,
        **process_kws) -> float:
    """Mean log error
    Parameters
    ----------
    ...
    """
    if preprocess:
        true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mle'], **process_kws)
    return float(np.mean(log1p(predicted) - log1p(true)))


def mod_agreement_index(true, predicted, preprocess: bool = True, j=1,
                        **process_kws) -> float:
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mod_agreement_index'], **process_kws)
    a = (np.abs(predicted - true)) ** j
...
def mpe(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    ...
    >>> mpe(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mpe'], **process_kws)
...
def mrae(true, predicted, preprocess: bool = True, benchmark: np.ndarray = None,
    ...
    >>> mrae(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mrae'], **process_kws)
    return float(np.mean(np.abs(_relative_error(true, predicted, benchmark))))


def norm_euclid_distance(true, predicted, preprocess: bool = True,
                         **process_kws) -> float:
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['norm_euclid_distance'], **process_kws)
...
def nrmse_range(true, predicted, preprocess: bool = True,
                **process_kws) -> float:
    ...
    >>> nrmse_range(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nrmse_range'], **process_kws)

    return float(rmse(true, predicted, preprocess=False) / (np.max(true) - np.min(true)))


def nrmse_ipercentile(true, predicted, preprocess: bool = True, q1=25, q2=75,
                      **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nrmse_ipercentile'], **process_kws)

    q1 = np.percentile(true, q1)
    q3 = np.percentile(true, q2)
    iqr = q3 - q1

    return float(rmse(true, predicted, preprocess=False) / iqr)


def nrmse_mean(true, predicted, preprocess: bool = True,
               **process_kws) -> float:
    ...
    >>> nrmse_mean(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['nrmse_mean'], **process_kws)
    return float(rmse(true, predicted, preprocess=False) / np.mean(true))


def norm_ae(true, predicted, preprocess: bool = True,
            **process_kws) -> float:
    ...
    >>> norm_ae(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['norm_ae'], **process_kws)
    error = true - predicted
    return float(np.sqrt(np.sum(np.square(error - mae(true, predicted, False))) / (len(true) - 1)))


def log_prob(true, predicted, preprocess: bool = True,
             **process_kws) -> float:
    """
    ...
    >>> log_prob(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['log_prob'], **process_kws)
...
def rmdspe(true, predicted, preprocess: bool = True,
           **process_kws) -> float:
    """
    ...
    >>> rmdspe(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rmdspe'], **process_kws)
    return float(np.sqrt(np.median(np.square(_percentage_error(true, predicted)))) * 100.0)


def rse(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    ...
    >>> rse(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rse'], **process_kws)
    return float(np.sum(np.square(true - predicted)) / np.sum(np.square(true - np.mean(true))))


def rrse(true, predicted, preprocess: bool = True,
         **process_kws) -> float:
    ...
    >>> p = np.random.random(10)
    >>> rrse(t, p)"""
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rrse'], **process_kws)
    return float(np.sqrt(rse(true, predicted, preprocess=False)))


def rae(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    ...
    >>> rae(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rae'], **process_kws)
    return float(np.sum(_ae(true, predicted)) / (np.sum(np.abs(true - np.mean(true))) + EPS))


def ref_agreement_index(true, predicted, preprocess: bool = True,
                        **process_kws) -> float:
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['ref_agreement_index'], **process_kws)
    a = np.sum(np.abs(predicted - true))
...
def rel_agreement_index(true, predicted, preprocess: bool = True,
                        **process_kws) -> float:
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rel_agreement_index'], **process_kws)
...
def relative_rmse(true, predicted, preprocess: bool = True,
                  **process_kws) -> float:
    """
    ...
    >>> relative_rmse(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['relative_rmse'], **process_kws)
    rrmse = rmse(true, predicted, preprocess=False) / np.mean(true)
    return float(rrmse)


def rmspe(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    """
    ...
    >>> rmspe(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rmspe'], **process_kws)
    return float(np.sqrt(np.mean(np.square(((true - predicted) / true)), axis=0)))


def rsr(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    """
    Moriasi et al., 2007.
    ...
    >>> rsr(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rsr'], **process_kws)
    return float(rmse(predicted=predicted, true=true, preprocess=False) / np.std(true))


def rmsse(true, predicted, preprocess: bool = True, seasonality: int = 1,
          **process_kws) -> float:
    ...
    >>> rmsse(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['rmsse'], **process_kws)
    error = true - predicted
    q = np.abs(error) / mae(true[seasonality:], _naive_prognose(true, seasonality), preprocess=False)
    return float(np.sqrt(np.mean(np.square(q))))


def sa(true, predicted, preprocess: bool = True,
       **process_kws) -> float:
    ...
    >>> sa(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['sa'], **process_kws)
    a = np.dot(predicted, true)
    b = np.linalg.norm(predicted) * np.linalg.norm(true)
    return float(np.arccos(a / b))


def sc(true, predicted, preprocess: bool = True,
       **process_kws) -> float:
    ...
    >>> sc(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['sc'], **process_kws)
...
def sga(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    ...
    >>> sga(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['sga'], **process_kws)
...
def smape(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    """
    ...
    >>> smape(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['smape'], **process_kws)
    _temp = np.sum(2 * np.abs(predicted - true) / (np.abs(true) + np.abs(predicted)))
    return float(100 / len(true) * _temp)


def smdape(true, predicted, preprocess: bool = True,
           **process_kws) -> float:
    """
    ...
    >>> smdape(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['smdape'], **process_kws)
    return float(np.median(2.0 * _ae(predicted=predicted, true=true) / ((np.abs(true) + np.abs(predicted)) + EPS)))


def sid(true, predicted, preprocess: bool = True,
        **process_kws) -> float:
    ...
    >>> sid(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['sid'], **process_kws)
...
def skill_score_murphy(true, predicted, preprocess: bool = True,
                       **process_kws) -> float:
    """
    Adopted from here_ .
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['skill_score_murphy'], **process_kws)
    # Calculate RMSE
    rmse2 = rmse(true, predicted, preprocess=False) ** 2
...
def std_ratio(true,
    ...
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['std_ratio'], **process_kws)
...
def umbrae(true, predicted, preprocess: bool = True, benchmark: np.ndarray = None,
    ...
    >>> umbrae(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['umbrae'], **process_kws)
    return mbrae(true, predicted, False, benchmark) / (1 - mbrae(true, predicted, False, benchmark))


def ve(true, predicted, preprocess: bool = True,
       **process_kws) -> float:
    """
    ...
    >>> ve(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['ve'], **process_kws)
    a = np.sum(np.abs(predicted - true))
    b = np.sum(true)
    return float(1 - (a / b))


def volume_error(true, predicted, preprocess: bool = True,
                 **process_kws) -> float:
    """
    ...
    >>> volume_error(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['volume_error'], **process_kws)
    # TODO written formula and executed formula are different.
    ve_ = np.sum(predicted - true) / np.sum(true)
    return float(ve_)


def wape(true, predicted, preprocess: bool = True,
         **process_kws) -> float:
    """
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['wape'], **process_kws)
    return float(np.sum(_ae(true, predicted) / np.sum(true)))


def watt_m(true, predicted, preprocess: bool = True,
           **process_kws) -> float:
    """Watterson's M.
    ...
    >>> watt_m(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['watt_m'], **process_kws)
    a = 2 / np.pi
    c = np.std(true, ddof=1) ** 2 + np.std(predicted, ddof=1) ** 2
    e = (np.mean(predicted) - np.mean(true)) ** 2
    f = c + e
    return float(a * np.arcsin(1 - (mse(true, predicted, preprocess=False) / f)))


def wmape(true, predicted, preprocess: bool = True,
          **process_kws) -> float:
    """
    ...
    >>> wmape(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['wmape'], **process_kws)
...
def norm_ape(true, predicted, preprocess: bool = True,
             **process_kws) -> float:
    ...
    >>> norm_ape(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['norm_ape'], **process_kws)
    return float(
        np.sqrt(np.sum(np.square(_percentage_error(true, predicted) - mape(true, predicted, preprocess=False))) / (
                    len(true) - 1)))


def mse(true, predicted, preprocess: bool = True, weights=None,
        **process_kws) -> float:
    """
    Mean Square Error
    ...
    >>> mse(t, p)
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['mse'], **process_kws)
    return float(np.average((true - predicted) ** 2, axis=0, weights=weights))


def variability_ratio(true, predicted, preprocess: bool = True,
                      **process_kws) -> float:
    """
    Variability Ratio
    ...
    """
    true, predicted = maybe_preprocess(preprocess, true, predicted, METRIC_TYPES['variability_ratio'], **process_kws)
